...
Distinct from *Shadows Instead of Borders* (depth ring on cards). For genuine elevation (a floating panel, a menu, a dragged item), real objects cast **two shadows**: a tight contact shadow and a soft ambient cast. One blurry shadow looks CGI.

```css
/* Resting elevation */
.elevated-1 {
  box-shadow:
    0 1px 2px rgba(0, 0, 0, 0.08),      /* contact: tight, darker */
    0 8px 24px rgba(0, 0, 0, 0.08);     /* ambient: wider, softer */
}

/* Higher elevation — object is farther from surface */
.elevated-2 {
  box-shadow:
    0 1px 2px rgba(0, 0, 0, 0.04),      /* contact fades (farther away) */
    0 16px 40px rgba(0, 0, 0, 0.12);    /* ambient grows + softens */
}
```

Rule of thumb as elevation increases: contact shadow **fades** (less contact with surface); ambient shadow **grows and softens**.

## `:focus-visible`, not `:focus`

`:focus` fires on every click, showing rings pointer users never asked for. `:focus-visible` only shows rings when focus came from keyboard navigation.

```css
/* Good — rings only for keyboard users */
.button:focus-visible {
  outline: 2px solid var(--accent);
  outline-offset: 2px;
}

/* Bad — ring flashes on every mouse click */
.button:focus {
  outline: 2px solid var(--accent);
}
```

For composite controls (a card wrapping an input + submit), use `:focus-within` on the container to light up the group when any descendant has focus.

## Match Browser Chrome to Dark Mode

Dark UI with a white scrollbar or white mobile browser chrome breaks the illusion instantly.

```html
<html style="color-scheme: dark">
  <head>
    <!-- Mobile browser chrome (Safari iOS, Chrome Android) -->
...
Use `color-scheme: light dark` on `<html>` when supporting both — the browser picks the right native form-control and scrollbar styling automatically. This fixes: scrollbar color, `<input type="date">` popovers, `<select>` dropdowns, mobile status bar background.
